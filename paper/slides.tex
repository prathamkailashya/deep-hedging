...
\subtitle{Novel Approaches Including Regime-Switching Ensemble (RSE) for Robust Tail Risk Control}
...
\begin{frame}{Main Results: CVaR Performance (10 Seeds, 50K Samples)}
\begin{table}
\centering
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{CVaR$_{95}$} $\downarrow$ & \textbf{Std P\&L} & \textbf{Entropic} & \textbf{Trade Vol.} & \textbf{CV (\%)} \\
\midrule
\textbf{RSE} & \textcolor{darkgreen}{$\mathbf{3.109 \pm 0.010}$*} & $0.456$ & $2.376$ & $1.379$ & $\mathbf{0.30}$ \\
...
\vspace{0.2cm}
\small{* Statistically significant vs. LSTM ($p < 0.0001$, Cohen's $d = -7.41$)}

\vspace{0.2cm}
\textbf{Key Finding:} RSE achieves \textbf{-3.29\%} improvement with \textbf{best stability} (CV=0.30\%)
\end{frame}
...
\toprule
\textbf{Model} & \textbf{CVaR$_{95}$} & \textbf{vs LSTM} & \textbf{$p$-value} & \textbf{Cohen's $d$} & \textbf{Sig.} \\
\midrule
...
\vspace{0.3cm}
\textbf{Bootstrap 95\% CIs:}
\begin{itemize}
    \item RSE: $[3.104, 3.115]$
    \item LSTM: $[3.204, 3.226]$
    \item Transformer: $[3.215, 3.252]$
\end{itemize}

\small{* Significant after Holm-Bonferroni correction}
\end{frame}

\begin{frame}{Key Finding 1: RSE Achieves Best Tail Risk}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{RSE vs. LSTM:}
\begin{itemize}
    \item CVaR$_{95}$: $\mathbf{-3.29\%}$ improvement
    \item $p < 0.0001$ (highly significant)
    \item Cohen's $d = -7.41$ (very large effect)
    \item Stability: CV $= 0.30\%$ (best)
\end{itemize}

\vspace{0.3cm}
\textbf{Interpretation:}\\
Regime-switching ensemble adapts to market conditions, providing robust tail risk control.
\end{column}
\begin{column}{0.5\textwidth}
\begin{tikzpicture}[scale=0.8]
\begin{axis}[
    ybar,
    ylabel={CVaR$_{95}$},
    symbolic x coords={RSE, LSTM, 3SCH, Trans},
    xtick=data,
    ymin=3.0, ymax=3.3,
    bar width=0.6cm,
    nodes near coords,
...
\addplot[fill=blue!60] coordinates {(RSE,3.109) (LSTM,3.215) (3SCH,3.219) (Trans,3.234)};
\end{axis}
\end{tikzpicture}
\end{column}
\end{columns}
...
\begin{frame}{Key Finding 2: Crisis Period Robustness (All Models)}
\begin{table}
\centering
\footnotesize
\begin{tabular}{lrrrrr}
\toprule
\textbf{SPY Scenario} & \textbf{LSTM} & \textbf{Trans.} & \textbf{RSE} & \textbf{W-DRO-T} & \textbf{3SCH} \\
\midrule
Normal 2019 ($\sigma=15\%$) & 20.28 & 14.47 & 16.90 & \textcolor{darkgreen}{\textbf{11.98}} & 15.79 \\
COVID ($\sigma=65\%$) & 86.84 & 64.80 & 69.76 & \textcolor{darkgreen}{\textbf{46.94}} & 69.25 \\
2008 Crisis ($\sigma=80\%$) & 100.88 & 72.71 & 84.57 & \textcolor{darkgreen}{\textbf{62.26}} & 88.43 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lrrrrr}
\toprule
\textbf{NIFTY Scenario} & \textbf{LSTM} & \textbf{Trans.} & \textbf{RSE} & \textbf{W-DRO-T} & \textbf{3SCH} \\
\midrule
Normal 2019 ($\sigma=14\%$) & 785.62 & 769.61 & 686.62 & 663.92 & \textcolor{darkgreen}{\textbf{622.26}} \\
COVID ($\sigma=55\%$) & 3028.93 & \textcolor{darkgreen}{\textbf{1644.19}} & 2592.97 & 2883.96 & 2465.71 \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.2cm}
\textbf{Key Insight:} \textbf{W-DRO-T} dominates US crisis scenarios; \textbf{3SCH/Transformer} excel in India
\end{frame}
...
\begin{enumerate}
    ...
    \vspace{0.2cm}
    \item \textbf{W-DRO-T achieves best crisis robustness}
    \begin{itemize}
        \item 46\% CVaR improvement in COVID, 38\% in 2008-type stress
        \item Lowest P\&L volatility (Std = 1.71 vs.\ 5.05 for LSTM)
    \end{itemize}
    
    \vspace{0.2cm}
    \item \textbf{Market-specific model selection is critical}
    \begin{itemize}
        \item W-DRO-T for US markets; 3SCH/Transformer for India
    \end{itemize}
    
    \vspace{0.2cm}
    \item \textbf{Excellent stability across all models} (CV $< 1\%$)
    \begin{itemize}
        \item Two-stage CVaR$\rightarrow$Entropic protocol is robust
    \end{itemize}
    
    \vspace{0.2cm}
    \item \textbf{Economic analysis} provides actionable insights
    \begin{itemize}
        \item 41\% capital savings with W-DRO-T under Basel III/IV
    \end{itemize}
\end{enumerate}
\end{frame}

\begin{frame}{Practical Recommendations}
\begin{tcolorbox}[colback=blue!5, colframe=darkblue, title=Model Selection Guidelines by Market]
\begin{enumerate}
    \item \textbf{US Markets (low transaction costs):}\\
    $\rightarrow$ \textbf{W-DRO-T} for crisis robustness (38\% better than LSTM in COVID)\\
    $\rightarrow$ RSE for synthetic-trained deployment (regime-adaptive)
    
    \vspace{0.2cm}
    \item \textbf{Indian Markets (high transaction costs):}\\
    $\rightarrow$ \textbf{3SCH} for normal periods (best CVaR = 622.26)\\
    $\rightarrow$ \textbf{Transformer} for crisis periods (46\% better than LSTM)
    
    \vspace{0.2cm}
    \item \textbf{For operational simplicity:}\\
    $\rightarrow$ LSTM baseline (strong, interpretable, CV $< 1\%$)
