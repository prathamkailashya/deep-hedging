...
\includegraphics[width=0.20\textwidth]{deliverable/poster_figs/iitr_logo.png}
...
Hedging derivatives is a central problem in quantitative finance. Black--Scholes delta hedging~\citep{black1973pricing} assumes continuous trading, zero transaction costs and market completeness; none of those hold on a real desk. Deep hedging~\citep{buehler2019deep} recasts the problem as stochastic optimal control with a neural-network policy, which is what lets us add transaction costs and risk preferences without rewriting the framework around them.

The empirical record is good, but three problems recur both in the literature and in our own benchmarks. \emph{Model uncertainty}: networks trained on a single calibrated Heston measure degrade once the deployed market drifts off that measure, and the worst losses pile up in stress regimes. \emph{Training instability}: the CVaR-then-entropic two-stage protocol of~\citet{kozyra2019deep} switches between qualitatively different risk measures at the boundary, which causes gradient jumps and occasional forgetting of the tail-aware solution. \emph{Regime dependence}: no single architecture wins in both calm and crisis conditions, yet production desks usually deploy a single hedger.
...
\begin{enumerate}[noitemsep,leftmargin=*]
    ...
    \item \textbf{3SCH:} a three-stage curriculum LSTM that interpolates between the CVaR and entropic objectives along a homotopy whose continuity follows from Berge's maximum theorem.
    \item \textbf{RSE:} a regime-switching ensemble that gates frozen LSTM, Transformer and signature experts through a learnable affinity matrix indexed by realised volatility, trend, momentum and jump statistics.
\end{enumerate}
...
\citet{buehler2019deep} replace analytic delta calculus with a neural-network policy trained to minimise a convex risk measure on the residual hedging error. Frictions, exotic payoffs and stochastic volatility all drop in without extra machinery. The recurring weakness is that training only ever sees one stochastic-volatility model, so performance falls sharply when the deployed market drifts. An LSTM trained on normal-regime Heston paths shows more than a four-fold inflation of $\mathrm{CVaR}_{95}$ under COVID-style stress; that brittleness is what motivates every architecture in this thesis.
...
\]
which the duality of \citet{blanchet2019quantifying} reduces to a gradient-norm regulariser. Training proceeds in three phases: CVaR pretraining, then DRO-annealed entropic fine-tuning ($\varepsilon: 0 \to 0.1$), then stress-test evaluation. The result is a \textbf{$46\%$ CVaR improvement on SPY COVID} and a \textbf{$41\%$ capital reduction under Basel~III/IV}.

\subsubsection{Three-Stage Curriculum Hedger (3SCH)}

3SCH replaces the abrupt CVaR-to-entropic switch with the homotopy loss
\[
\mathcal{L}(\alpha) = \alpha\, \cvar_{0.95} + (1-\alpha)\, \rho_\lambda,
\quad \alpha: 0.8 \to 0.2,
\]
which keeps the transition between the two objectives smooth. The architecture is most useful in high-cost regimes --- in this study, the Indian markets --- and attains the best \textbf{NIFTY CVaR$_{95}$ of 622.26}.

\subsubsection{Regime-Switching Ensemble (RSE)}

RSE combines LSTM, Transformer and Signature hedgers under a regime-aware gate. A classifier maps the regime features (RV, trend, momentum, jumps) to $K=4$ regimes, and an affinity matrix $A \in \mathbb{R}^{4 \times 3}$ turns regime probabilities into model weights. The result is \textbf{CVaR$_{95} = 3.109 \pm 0.010$} (\textbf{$-3.29\%$ vs.\ LSTM}, \textbf{$p<0.0001$}, \textbf{$d=-7.41$}) with the lowest cross-seed CV in the table (\textbf{$0.30\%$}).
%===============================================================
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{figures/03_heston_parameter_sensitivity.pdf}
...
The barrier and digital payoffs are not differentiable in $S_T$, so the hedger cannot lean on a smooth Black--Scholes-style delta and must learn the discontinuity directly from path data. The Asian payoff depends on the entire path, which favours architectures with a path-aware backbone. The basket call is genuinely multi-asset: the hedge set contains all three underliers and the cross-asset correlation enters the optimal policy. The same payoff battery is used in identical form across both the synthetic-Heston and real-data evaluations (Chapters~\ref{sec:results} and~\ref{sec:crisis}).
...
\textbf{KL ball}: requires $\QQ \ll \PP$, adversary can only \emph{reweight} existing scenarios. \textbf{Wasserstein ball}: does \emph{not} require absolute continuity, adversary can \emph{move} mass to new regions. For crisis hedging, Wasserstein is more appropriate because distributional shifts involve \emph{new} volatility/stress regimes (COVID $\sigma = 65\%$) not seen during training ($\sigma \approx 20\%$).
\end{remark}
...
\caption{RSE regime features along a representative simulated path: multi-window realised volatility, the SMA-crossover trend indicator, bipower variation and the jump ratio (with the jump-detection threshold marked). These are the inputs the regime classifier $f_\phi$ consumes at every time step.}
...
The two-stage \citet{kozyra2019deep} protocol switches abruptly from $\cvar_{0.95}$ to $\rho_\lambda$ between Stage~1 and Stage~2, which produces gradient discontinuities, occasional forgetting of the tail-aware solution, and convergence to local minima. In the curriculum-learning view of~\citet{bengio2009curriculum}, CVaR is the ``easier'' tail-focused objective and entropic is the ``harder'' global one. We replace the abrupt switch with a homotopy whose regularity is provable.  Concretely, we use the convex combination
\begin{equation}
    \mathcal{L}_{\mathrm{mixed}}(\alpha; \theta) = \alpha \cdot \cvar_{0.95}(-\mathrm{P\&L}^\theta) + (1-\alpha) \cdot \rho_\lambda(-\mathrm{P\&L}^\theta)
\end{equation}
which is a convex risk measure for each fixed $\alpha$ and a scalarisation of the underlying bi-objective problem.  Berge's maximum theorem (Appendix~\ref{app:theory}) yields upper hemicontinuity of $\alpha\mapsto\Theta^*(\alpha)$, justifying the homotopy continuation.  3SCH inherits the LSTM architecture (2 layers, $h=50$, $\sim$54K parameters) without modification: any improvement is therefore attributable to the schedule and not to the capacity.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{figures/17_training_schedule_3sch.pdf}
...
Heston-trained policies degrade more than four-fold in $\cvar_{95}$ when deployed under the COVID-style volatility ($\sigma\approx65\%$) they were never trained on. W-DRO-T closes that gap by minimising worst-case risk over a Wasserstein ball~\citep{esfahani2018data} around the training measure:
\begin{equation}
\theta^* = \argmin_\theta \sup_{\QQ:\,W_p(\QQ,\PP)\le\varepsilon}
\E_\QQ\bigl[\rho(\mathrm{P\&L}(\delta^\theta))\bigr],
\label{eq:dro_obj}
\end{equation}
with ambiguity set $\mathcal{P}^W(\PP;\varepsilon) = \{\QQ:W_p(\QQ,\PP)\le\varepsilon\}$ and radius $\varepsilon>0$.  Compared with the KL ball implicit in entropic risk --- which only re-weights existing scenarios because it requires $\QQ\ll\PP$ --- the Wasserstein ball can \emph{move} probability mass to new regions of the input space and so bounds risk under genuine support shifts such as a regime change in volatility~\citep{esfahani2018data}.
...
Synthetic Heston paths cannot, on their own, decide whether an architecture wins because of genuine inductive bias or because the simulator happens to be friendly to it. We therefore augment the experimental matrix with a real-market pipeline that consumes daily OHLCV from \texttt{yfinance} and slices it into crisis-labelled hedging episodes. The pipeline writes Parquet caches under \texttt{outputs/cache/} so subsequent runs avoid network calls; if a download fails (network restriction, ticker delisted), a deterministic synthetic generator with regime-aware volatility levels takes over so the experimental harness always finishes.
...
Synthetic-only evaluation has three concrete gaps that motivate the real-data pipeline. Calibrated Heston paths under-represent jumps: real index returns admit discontinuous moves on macro news days, and \citet{cont2001empirical} document excess kurtosis that Gaussian diffusion cannot reproduce. The cross-asset dependence structure is itself regime-dependent --- correlations rise from a calm baseline near $0.55$ to crisis levels above $0.75$ during COVID and the GFC, a feature the Bates extension captures but which still needs validation against the empirical record. And transaction-cost asymmetries (US 3~bps vs.\ Indian NIFTY 18~bps including STT and slippage) are not a property of the dynamics at all; they enter through the cost ledger of the P\&L equation, so the relative ranking of hedgers under different friction regimes only becomes legible once we run with realistic cost vectors~\citep{leland1985option,whalley1997asymptotic}. Anything heading for production therefore has to be tested on real-market crisis windows, not just on the synthetic Heston battery.
...
We use 10 seeds $s \in \{42,142,\dots,942\}$ controlling data generation, weight initialisation, batch ordering, and dropout. This ensures variability and reflects model robustness.
...
At seed $s$, for model $A$ vs.\ LSTM, define $d_s=\hat\mu_s^A-\hat\mu_s^{\mathrm{LSTM}}$. Test $H_0:\E[d]=0$ via paired $t$-test, or Wilcoxon if normality fails (Shapiro--Wilk, $\alpha=0.05$).

\subsubsection{Multiple Testing Correction}

Holm--Bonferroni~\citep{holm1979simple}: sort $p_{(1)}\le\cdots\le p_{(K)}$ and reject $H_{0,(j)}$ if $p_{(j)}\le \alpha/(K-j+1)$, $\alpha=0.05$.
...
RSE attains the lowest CVaR$_{95}$ at $3.109 \pm 0.010$, a $3.29\%$ improvement over LSTM. It also has the lowest coefficient of variation ($0.30\%$), so the ensemble combination stabilises performance across random seeds. The mean P\&L is essentially identical across architectures ($\approx -2.277$), so the gain is not coming from a different bias.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/18_cvar95_comparison_with_ci.pdf}
...
RSE's 95\% CI ($[3.103, 3.115]$) does not overlap with any of the other models, so the in-distribution gap on simulated data is unambiguous.

\begin{figure}[H]
\centering
\includegraphics[width=0.49\textwidth]{figures/20_cvar_seed_consistency.pdf}\hfill
\includegraphics[width=0.49\textwidth]{figures/22_cvar_cv_percent.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{figures/generated/effect_size_heatmap.pdf}
...
Tables~\ref{tab:medium_results} and~\ref{tab:paired_medium} read as follows. The causal Transformer wins or ties on every single-asset European, Asian, barrier and digital payoff across every scenario, with $\Delta_{\mathrm{CVaR}}$ between $-9.3\%$ and $-19.8\%$ versus LSTM and Holm--Bonferroni-corrected $p$-values below $0.02$ on Asian and digital payoffs in COVID and GFC. W-DRO-T is essentially tied with the Transformer on the synthetic in-distribution suite, but pulls ahead on the down-and-in put under COVID and GFC, where the gradient-norm regulariser converts into smaller tail risk under heavy distributional shift (Cohen's $|d|$ between $8$ and $35$ on the Asian/GFC cells). The basket-call row of Table~\ref{tab:medium_results} is the one cell where 3SCH and LSTM approach the Transformer's CVaR; this is the friction-driven regime in which the curriculum's low-turnover policy stays competitive, and the deployment recommendations in Chapter~\ref{sec:economic} lean on that.

\paragraph{Risk--cost frontier.}
Figure~\ref{fig:turnover} plots the per-seed CVaR$_{95}$ of every (model, scenario, task) cell against its turnover. As predicted by classical friction-aware hedging theory~\citep{leland1985option,whalley1997asymptotic}, the frontier is non-trivial: the lowest-CVaR points (Transformer / W-DRO-T) sit at higher turnover, while the lowest-turnover points (3SCH and LSTM) trade tail risk for trade frequency. The optimal point depends on the friction regime, which is what drives the market-specific deployment guidelines of Section~\ref{sec:selection}.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{jaws_research/outputs/figures/turnover_vs_cvar_quick_v1.pdf}
\caption{Risk--cost frontier across all single-asset (model, scenario, task) cells of the extended-suite benchmark.  Each point corresponds to one seed.  W-DRO-T and Transformer occupy the low-CVaR end of the frontier; 3SCH and LSTM occupy the low-turnover end.  Source: \texttt{jaws\_research/outputs/figures/turnover\_vs\_cvar\_quick\_v1.pdf}.}
\label{fig:turnover}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{jaws_research/outputs/figures/crisis_ratio_quick_v1.pdf}
\caption{Crisis-to-normal CVaR$_{95}$ ratio per model on the extended European-call cell.  Lower is more robust.  W-DRO-T attains the smallest COVID ratio; the LSTM baseline degrades the most.  Source: \texttt{jaws\_research/outputs/figures/crisis\_ratio\_quick\_v1.pdf}.}
...
W-DRO-T wins every SPY scenario: $41\%$ better than LSTM in the normal regime, $46\%$ in COVID, $38\%$ in 2008. The DRO gradient penalty buys distributional robustness exactly when capital constraints bind hardest.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{figures/23_spy_crisis_stress_testing.pdf}
...
W-DRO-T has the lowest P\&L standard deviation across all scenarios: $1.71$ in normal (vs $5.05$ for LSTM, $-66\%$) and $6.22$ in COVID (vs $22.18$, $-72\%$). The crisis-to-normal Std ratio of $3.64\times$ is the best in the table, so DRO regularisation also smooths the P\&L distribution.
...
The qualitative pattern from synthetic Heston paths --- Transformer and W-DRO-T at the low-CVaR end of the frontier, 3SCH at the low-turnover end --- carries over when the underlying price process is real. The crisis-to-calm CVaR ratio remains the cleanest discriminant: hedgers with the gradient-norm penalty degrade less than the LSTM baseline when the market drifts off the training window, which we read as direct empirical confirmation of the Wasserstein-DRO bound of Corollary~\ref{cor:grad_penalty}.
...
\chapter{Economic, Regulatory, and Governance Implications}
\label{sec:economic}
...
W-DRO-T saves $41\%$ of regulatory capital. On a \$10B derivatives book that is roughly \$332M of annual capital reduction. The saving widens further under COVID-style stress, where W-DRO-T's capital proxy is $46\%$ below LSTM's --- exactly when the constraint binds most tightly. RSE saves $16.7\%$ (\$6.76M vs.\ \$8.11M) and does so with materially lower trading costs.
...
The CVaR-per-trade ratio is the clearest efficiency summary: the Transformer delivers $5.0$ CVaR units per trade against LSTM's $33.8$. For India at $18$~bps, 3SCH's \$157K cost against W-DRO-T's \$369K makes 3SCH optimal --- the \$212K saving exceeds the marginal risk benefit of DRO regularisation.
...
Indian books (NIFTY/BANKNIFTY) carry an all-in $\sim$$18$~bps cost: STT ($0.05\%$ on options premium), $2$--$3$~bps of stamp duty and exchange fees, $5$~bps of slippage on the wider quotes, plus the operational frictions of discrete $50$-point strikes and SEBI position limits. These frictions favour 3SCH in normal regimes (CVaR $=622.26$, transaction cost \$157K per \$100M notional) and the plain Transformer as a crisis override (COVID CVaR $=1644.19$). On the Pareto frontier the four hedgers trace a tail-risk-vs-turnover trade-off: W-DRO-T ($11.98$ CVaR, $2.05$ volume), RSE ($16.90$, $1.06$), 3SCH ($15.79$, $0.87$) and LSTM ($20.28$, $0.60$). On a \$10B RSE desk the implied annual capital saving against LSTM is $\sim$\$135M, against an incremental US transaction-cost charge of \$14M, so the net benefit from capital alone is on the order of \$121M.

\subsection{Model Selection Guidelines}
\label{sec:selection}

The deployment recipe follows directly from the friction--regime trade-off. In low-friction US books (3~bps), W-DRO-T is the default: a $41\%$ capital saving at \$10B notional (\$332M per year) plus a $46\%$ COVID CVaR improvement together justify the higher trading volume. In high-friction Indian books (18~bps), 3SCH takes over: turnover of $0.90$ minimises STT and stamp-duty exposure and yields the lowest NIFTY normal CVaR ($622.26$); when the market enters a crisis the Transformer is a sensible override (COVID CVaR $1644.19$). RSE is the natural choice for desks operating under genuine regime uncertainty: it gives the best overall in-distribution CVaR ($3.109$), the lowest cross-seed CV ($0.30\%$) and an interpretable regime classifier. LSTM and 3SCH remain the operational defaults for compute-constrained or audit-heavy environments: 3SCH uses an identical LSTM architecture and only changes the training schedule, and both qualify for hedge accounting with minimal documentation overhead.
...
\subsubsection{Validation, Versioning and Rollback}

Independent validation rests on four pillars common to all five hedgers: backtesting on rolling $252$-day windows; crisis stress tests under COVID ($\sigma\approx65\%$) and GFC ($\sigma\approx80\%$); model-specific sensitivity analyses ($\varepsilon$ for W-DRO-T, the annealing schedule for 3SCH, $K$ for RSE); and a $10$-seed stability check ($\mathrm{CV}<1\%$ for every architecture). W-DRO-T's smaller crisis-to-normal CVaR$_{95}$ ratio ($3.92\times$ vs.\ $4.28\times$ for LSTM) suggests retraining can happen less often; in practice we recommend monthly retraining with fresh Heston-calibrated parameters, quarterly full revalidation including crisis stress tests, and Git-based version control of checkpoints, with automated monitoring of CVaR$_{95}$ and gradient-norm drift. Each architecture admits a zero-cost rollback: setting $\varepsilon=0$ recovers a vanilla Transformer for W-DRO-T, $T_2=0$ recovers two-stage training for 3SCH, and $w_m\equiv1/M$ recovers an equal-weight ensemble for RSE. The ultimate contingency is the LSTM baseline. Automated rollback triggers: (i)~CVaR$_{95}$ exceeding $1.5\times$ the LSTM baseline for five consecutive days; (ii)~gradient norm exceeding $3\times$ the training average; (iii)~backtesting exceptions above the Basel traffic-light threshold of four in a $250$-day window.

\subsection{Regulatory and Accounting Implications}
\label{sec:regulatory}

Under the FRTB Internal Models Approach, expected shortfall at $97.5\%$ replaces VaR as the primary risk measure ($\mathrm{ES}_{97.5}\approx 1.12\,\mathrm{CVaR}_{95}$); the capital charge is $\mathrm{ES}\times m_s$ with $m_s\ge1.5$. W-DRO-T's lower P\&L volatility (Std P\&L of $1.71$ vs.\ $5.05$ for LSTM) translates into a lower backtesting-exception rate ($\sim$$1$/year vs.\ $\sim$$2.5$/year), which keeps the desk on the IMA and away from the punitive Standardised Approach. IFRS~7 requires disclosure of model architecture, training methodology, backtesting results with confidence intervals and governance procedures, including model-specific items: the W-DRO-T robustness radius $\varepsilon$ and its economic interpretation, the RSE ensemble structure and regime classifier, and the 3SCH three-stage schedule. For Indian markets, the SEBI algorithmic-trading guidelines require automated-strategy documentation, and Ind-AS 109 mirrors IFRS~9 with additional derivative disclosures. Combined with the $18$~bps all-in cost, these constraints make 3SCH the natural NIFTY choice.
...
Three benchmarks anchor the empirical claims of the thesis. On the in-distribution ten-seed European-call benchmark, RSE attains the lowest tail risk ($\cvar_{95}=3.109\pm0.010$, $-3.29\%$ vs.\ LSTM, $p<0.0001$, $d=-7.41$) and the tightest cross-seed coefficient of variation ($0.30\%$). On the market-calibrated Heston scenarios, W-DRO-T dominates the SPY single-asset European call across regimes ($\cvar_{95}=11.98/46.94/62.26$ in normal/COVID/GFC, $41/46/38\%$ improvements over LSTM), while 3SCH wins the NIFTY normal regime ($622.26$) on the strength of its low turnover ($0.90$ vs.\ W-DRO-T's $2.70$). On the extended medium-scale benchmark (Section~\ref{sec:extended_suite}, $5{\times}7{\times}4$ cells, three seeds), the causal Transformer or W-DRO-T attains the lowest $\cvar_{95}$ on every single-asset payoff in every scenario, with paired-test $p<0.02$ and Cohen's $|d|$ up to $34$ on GFC Asian; the basket call is the lone exception, where RSE wins calm and W-DRO-T wins COVID. Walk-forward real-market windows on SPY/QQQ/NIFTY-50 reproduce this pattern. Translated into Basel-IV capital, W-DRO-T saves \$$3.32$M per \$$100$M notional in normal SPY and more under stress --- roughly \$$332$M per year on a \$$10$B desk (Tables~\ref{tab:medium_results}, \ref{tab:paired_medium}, \ref{tab:real_results}, \ref{tab:capital}).

\subsection{Discussion}
\label{sec:concl_discussion}

The empirical pattern splits along two axes: transaction cost and regime stability. In low-friction US books, the gradient-norm penalty pays for its modest training-time cost; W-DRO-T delivers $38$--$46\%$ CVaR$_{95}$ gains in crises and a $41\%$ Basel-IV capital saving, which together dominate deployment cost at $3$~bps. In high-friction Indian books the same active strategy is taxed by the $18$~bps all-in cost, and 3SCH's low turnover flips the cost-adjusted ranking in its favour. RSE sits between the two, paying for its three-fold inference cost with seed stability ($\mathrm{CV}=0.30\%$) and a transparent audit trail. In calm regimes ($\sigma\approx15\%$) the architectures fall within a percentage point of each other, so LSTM remains the operational default; the gap only opens once the regime moves. On the Pareto front, RSE wins the risk--cost trade-off at $\sim 15$~min training, W-DRO-T pays $\sim 119$~min for crisis performance, and LSTM/3SCH own the low-compute corner. Adding architecture is worth it only when the expected risk reduction beats the marginal operational cost.

\subsection{Limitations}
\label{sec:limitations}

The Heston simulator does not reproduce flash crashes, liquidity dry-ups, or regime-dependent correlations; the Bates extension adds jumps but not stochastic correlation. The exotic suite is capped at basket dimension $d=3$ for Apple-Silicon tractability, so $d\gtrsim 30$ portfolio hedging is out of scope and the cross-asset correlation is held constant within a regime. Real-market data are daily \texttt{yfinance} OHLCV; intraday microstructure and order-book impact are not modelled, and when the feed is unavailable the synthetic fallback matches volatility but not the full correlation structure, so fallback numbers are conservative. Hyperparameter search is restricted to the key axes ($\varepsilon$, $\alpha$ schedule, $K$); W-DRO-T's MATH-SDPA requirement disables flash-attention; and the gradient penalty is first-order, with degraded tightness for $\varepsilon\gtrsim 0.2$. Architecture-specific caveats: RSE underperforms W-DRO-T in dedicated crisis scenarios ($46.94$ vs $69.76$ on SPY COVID) and triples inference compute; 3SCH's optimal schedule is market-dependent; and W-DRO-T's higher turnover would amplify market impact, which is not modelled here.

\subsection{Future Work}
\label{sec:future}

The most immediate extension is online adaptive hedging: the walk-forward pipeline of Section~\ref{sec:real_walkforward} can update the RSE gating matrix on streaming data without retraining the frozen base hedgers. Scaling the multi-asset suite to $d\gtrsim 10$ requires sparse or block-diagonal attention and a cross-asset DRO penalty. Market-impact modelling along the lines of~\citet{pickard2025rl} becomes load-bearing at large notionals. Attention-pattern analysis on the Transformer would expose which time steps and features drive hedging decisions. Cross-market transfer learning --- pretrain on liquid US markets, fine-tune the RSE regime classifier on NIFTY --- reuses the existing pipeline. A natural hybrid is 3SCH-DRO, combining the 3SCH curriculum with the W-DRO-T gradient-norm penalty, with $\varepsilon$ tuned on crisis holdouts. Finally, RL frameworks~\citep{cao2021deep,haarnoja2018soft,huang2025sac,neagu2025drl} and rough-path signatures~\citep{lyons1998differential,kidger2020signatory} address non-Markovian dynamics beyond level-$2$ truncation.

\subsection{Closing Remarks}
\label{sec:concl_closing}

The thesis makes three contributions. \emph{Methodologically}, three architectural responses to documented weaknesses of two-stage deep hedging --- W-DRO-T, 3SCH and RSE --- each grounded in distinct theory (Blanchet--Murthy duality~\citep{blanchet2019quantifying,esfahani2018data}, Berge's maximum theorem, and the ensemble variance decomposition of~\citet{dietterich2000ensemble}); all three are auditable and admit zero-cost rollback. \emph{Empirically}, a unified driver evaluates the five hedgers on seven payoffs, four scenarios, multi-asset Bates jump-diffusion and walk-forward windows on SPY, QQQ and NIFTY-50: Transformer/W-DRO-T win every single-asset cell with significant margins, 3SCH wins NIFTY-style high-friction calm regimes, and RSE wins on cross-seed stability and on basket-call calm. \emph{Operationally}, the released \texttt{jaws\_research/} pipeline ships simulators, an online data feed, the training driver, a bootstrap statistical layer and auto-refreshing result blocks; it runs on Apple Silicon (MPS) without modification and is $30$--$50\%$ faster on NVIDIA hardware. No architecture wins everywhere: deploy W-DRO-T for low-friction US books, 3SCH for high-friction Indian books, and RSE under genuine regime uncertainty. The Wasserstein-DRO penalty is free at inference, and its FRTB capital benefit justifies deployment whenever transaction costs do not penalise active trading.
...
deep_hedging_new/
  ...
  jaws_research/                # Extended pipeline (Sec.6.4 / Sec.7.7 / App.H)
    ...
    outputs/{runs,figures,
             tables,cache,logs} # Pickled results, generated figures
