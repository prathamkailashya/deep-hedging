...
Real-world derivative hedging faces multiple frictions absent from classical theory:
\begin{itemize}[noitemsep]
    \item \textbf{Transaction costs:} SPY $\approx 3$~bps all-in; NIFTY $\approx 18$~bps (including Securities Transaction Tax at 0.05\%, stamp duty 2--3~bps, wider spreads at 5~bps slippage)
    \item \textbf{Discrete rebalancing:} Daily hedging at $N=30$ time steps for 30-day options
    \item \textbf{Model uncertainty:} Heston stochastic volatility parameters shift across market regimes; calibration error persists
    \item \textbf{Incomplete markets:} Stochastic volatility renders perfect replication impossible
    \item \textbf{Position limits:} Regulatory and risk constraints bound maximum hedge ratios
\end{itemize}

Indian equity options (NIFTY, BANKNIFTY) feature particularly challenging conditions: STT of 0.05\% on options premium, wider bid-ask spreads, discrete strikes at 50-point NIFTY intervals, and SEBI position limits. This cost differential fundamentally changes the optimal hedging strategy, favouring low-turnover models.
...
Delta bounding via $\delta_k = \delta_{\max}\tanh(\cdot)$ prevents unbounded positions and stabilises gradient flow during backpropagation through time (BPTT). For the 30-step hedging horizon ($N=30$), vanilla RNNs suffer gradient decay of order $\sim 0.95^{30} \approx 0.21$. The LSTM's gated cell state mitigates this: when the forget gate $f_k \approx 1$, $\partial c_k/\partial c_{k-1} \approx I$, allowing gradients to flow unattenuated. Gradient clipping at $\|\nabla\| \le 5.0$ provides additional stability.
...
3SCH & Training instability & Mixed-loss homotopy & Berge's maximum thm. & \citep{kozyra2019deep} \\
RSE & Regime dependence & Gated ensemble & Ensemble variance reduction & Gap analysis \\
RVSN & Rough volatility & Adaptive signatures & \citep{abijaber2023hedging} & Exploratory \\
SAC-CVaR & RL hedging & Constrained SAC & \citep{huang2025sac} & Exploratory \\
\bottomrule
\end{tabular}
\end{table}
...
For $n$ i.i.d.\ samples: $\E[W_1(\hat\PP_n, \PP)] = O(n^{-1/d})$, while $\E[\mathrm{KL}(\hat\PP_n \| \PP)] = +\infty$ for empirical vs.\ continuous measures. Wasserstein has a well-defined finite-sample convergence rate, making it the natural choice for data-driven DRO~\citep{esfahani2018data}.
\end{proposition}
