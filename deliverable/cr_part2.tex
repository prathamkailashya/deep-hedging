...
The LSTM hedger of~\citet{kozyra2019deep} processes $[I_k;\,\delta_{k-1}] \in \R^{d_{\mathrm{in}}+1}$ at each step $k$:
\begin{align}
    f_k &= \sigma(W_f [I_k;\,\delta_{k-1};\,h_{k-1}] + b_f), \\
    i_k &= \sigma(W_i [I_k;\,\delta_{k-1};\,h_{k-1}] + b_i), \\
    \tilde{c}_k &= \tanh(W_c [I_k;\,\delta_{k-1};\,h_{k-1}] + b_c), \\
    c_k &= f_k \odot c_{k-1} + i_k \odot \tilde{c}_k, \\
    o_k &= \sigma(W_o [I_k;\,\delta_{k-1};\,h_{k-1}] + b_o), \\
    h_k &= o_k \odot \tanh(c_k), \\
    \delta_k &= \delta_{\max} \cdot \tanh(W_{\delta} h_k + b_\delta),
\end{align}
where $\sigma$ is sigmoid, $\odot$ is element-wise multiplication, $(h_k, c_k)$ are hidden/cell states, $\delta_{\max}=1.5$. Architecture: 2 LSTM layers, hidden dimension 50, $\sim$54K parameters. Training: $\sim$273s/seed.

The cell state gradient factors as $\frac{\partial c_k}{\partial c_{k-1}} = f_k + \frac{\partial f_k}{\partial c_{k-1}} \odot c_{k-1} + \frac{\partial(i_k \odot \tilde{c}_k)}{\partial c_{k-1}}$. When $f_k \approx 1$, gradients flow unattenuated---critical for $N=30$ steps where vanilla RNNs suffer decay $\sim 0.95^{30} \approx 0.21$.

\subsubsection{Transformer Hedger Architecture}

\textbf{Input embedding.} $X_0[k] = W_{\mathrm{in}} I_k + \mathrm{PE}(k) \in \R^{d_{\mathrm{model}}}$ with sinusoidal positional encoding $\mathrm{PE}(k)_{2i} = \sin(k / 10000^{2i/d_{\mathrm{model}}})$, $\mathrm{PE}(k)_{2i+1} = \cos(k / 10000^{2i/d_{\mathrm{model}}})$.

\textbf{Scaled dot-product attention.} For head $h \in \{1,\ldots,H\}$, $d_h = d_{\mathrm{model}}/H$:
\begin{align}
    Q_h = X W_h^Q, \quad K_h = X W_h^K, \quad V_h = X W_h^V, \\
    \mathrm{Attn}_h(X) = \mathrm{softmax}\!\Bigl(\frac{Q_h K_h^\top}{\sqrt{d_h}} + M_{\mathrm{causal}}\Bigr) V_h,
\end{align}
where $[M_{\mathrm{causal}}]_{ij} = 0$ if $i \ge j$, $-\infty$ if $i < j$ (non-anticipativity).

\textbf{Multi-head attention and FFN:}
\begin{align}
    \mathrm{MHA}(X) &= \mathrm{Concat}(\mathrm{Attn}_1, \ldots, \mathrm{Attn}_H)\, W^O, \\
    X' &= \mathrm{LayerNorm}(X + \mathrm{MHA}(X)), \\
    X_l &= \mathrm{LayerNorm}(X' + \mathrm{FFN}(X')),
\end{align}
with $\mathrm{FFN}(x) = W_2\,\mathrm{GELU}(W_1 x + b_1) + b_2$, $W_1 \in \R^{d_{\mathrm{model}} \times d_{\mathrm{ff}}}$, $W_2 \in \R^{d_{\mathrm{ff}} \times d_{\mathrm{model}}}$.

\textbf{Output:} $\delta_k = 1.5 \cdot \tanh(W_o X_L[k] + b_o)$.

Architecture: $d_{\mathrm{model}}=64$, $H=4$, $L=3$, $d_{\mathrm{ff}}=256$, dropout $0.1$, $\sim$85K params, $\sim$7534s/seed.
...
\paragraph{Transformer Encoder with Causal Masking}
